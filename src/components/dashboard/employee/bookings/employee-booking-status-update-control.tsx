...
import {
  ...
  type EmployeeUpdatableBookingStatus,
} from "@/lib/dashboard/employee/bookings/employee-booking-status-actions";

const STATUS_OPTIONS: EmployeeUpdatableBookingStatus[] = [
  "IN_PROGRESS",
  "COMPLETED",
];

type EmployeeBookingStatusUpdateControlProps = {
  bookingId: string;
...
export function EmployeeBookingStatusUpdateControl({
  bookingId,
  currentStatus,
  compact = false,
}: EmployeeBookingStatusUpdateControlProps) {
  const [nextStatus, setNextStatus] = useState<EmployeeUpdatableBookingStatus | "">("");
  ...
  const handleConfirm = async () => {
    ...
    setIsPending(true);
    const result = await updateEmployeeBookingStatusAction(bookingId, nextStatus);
    setIsPending(false);
    ...
    <>
      ...
      >
        <select
          value={nextStatus}
          onChange={(event) =>
            setNextStatus(event.target.value as EmployeeUpdatableBookingStatus | "")
          }
